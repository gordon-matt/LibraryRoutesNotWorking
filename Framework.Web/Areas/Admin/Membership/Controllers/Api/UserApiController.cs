...
namespace Framework.Web.Areas.Admin.Membership.Controllers.Api
{
    public class UserApiController : ODataController
    {
        ...
        public virtual async Task<IActionResult> Get(ODataQueryOptions<FrameworkUser> options)
        {
            if (!CheckPermission(FrameworkWebPermissions.MembershipUsersRead))
            {
                return Unauthorized();
            }
            ...
            var response = await Task.FromResult((results as IQueryable<FrameworkUser>).ToHashSet());
            return Ok(response);
        }
        ...
        public virtual async Task<IActionResult> Put([FromODataUri] string key, [FromBody] FrameworkUser entity)
        {
            if (entity == null)
            {
                return BadRequest();
            ...
            if (!CheckPermission(FrameworkWebPermissions.MembershipUsersWrite))
            {
                return Unauthorized();
            }
        ...
        public virtual async Task<IActionResult> Post([FromBody] FrameworkUser entity)
        {
            if (entity == null)
            {
                return BadRequest();
        ...
        public virtual async Task<IActionResult> Patch([FromODataUri] string key, Delta<FrameworkUser> patch)
        {
            if (!CheckPermission(FrameworkWebPermissions.MembershipUsersWrite))
            {
                return Unauthorized();
            ...
            FrameworkUser entity = await Service.GetUserById(key);
            if (entity == null)
            {
                return NotFound();
            }
        ...
        public virtual async Task<IActionResult> GetUsersInRole(
            ...
            ODataQueryOptions<FrameworkUser> options)
        {
            if (!CheckPermission(FrameworkWebPermissions.MembershipUsersRead))
            {
                return Unauthorized();
